defmodule PrologParser do
  ...
  defp go_next([head | tokens], :identifier) do
    ...
    recursive_go_into_parentheses = fn tokens ->
      {left, right} = Enum.split(tokens, 1 + Enum.find_index(tokens, (fn x -> x == :rparen end)))
  ...
  def check_syntax(def_string) do
    ...
    {:ok, tokens, _} = def_string |> String.to_charlist |> :lexer.string
    ...
    handle_result.(parse_lexems(tokens))
  end
end
